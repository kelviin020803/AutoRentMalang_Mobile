...
class DaftarMobilPage extends StatefulWidget {
  final bool useDio;
  const DaftarMobilPage({super.key, this.useDio = false});
...
class _DaftarMobilPageState extends State<DaftarMobilPage> {
  ...
  void initState() {
    super.initState();
    // fetch according to choice
    if (widget.useDio) {
      controller.fetchWithDio();
    } else {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title:
            Text(widget.useDio ? 'Daftar Mobil (Dio)' : 'Daftar Mobil (HTTP)'),
        centerTitle: true,
      ),
      body: Column(
        children: [
          ...
          Expanded(
            child: Obx(() {
              ...
              if (list.isEmpty && !controller.loading.value) {
                return Center(
                  child:
                      Text('Tidak ada data. Tekan tombol refresh jika perlu.'),
                );
              }
